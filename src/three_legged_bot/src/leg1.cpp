         ...
         std::shared_ptr<core_interfaces::srv::UiMotor::Response> response)
{
  ...
  semaphore.acquire();
  RCLCPP_INFO(rclcpp::get_logger("rclcpp"), "Semaphore acquired.");
  RCLCPP_INFO(rclcpp::get_logger("rclcpp"), "Leg1Enterance");
  RCLCPP_INFO(rclcpp::get_logger("rclcpp"), "Received request: %d", request->leg1);
  ...
  int m7offset=30;
	...
	}else if (pos == 99) {  // Add a new case for testing motor 1
    int speed = 10;  // Adjust the speed to a low value for testing
    int angle = 50; // Small angle to easily observe movement

    RCLCPP_INFO(rclcpp::get_logger("rclcpp"), "Sending command to motor...");
	universialControl(13, 1, angle, speed, 0, 0, serial);
	RCLCPP_INFO(rclcpp::get_logger("rclcpp"), "Command sent.");

	RCLCPP_INFO(rclcpp::get_logger("rclcpp"), "Reading response...");
	auto response = readResponse(serial, buf, ec);
	RCLCPP_INFO(rclcpp::get_logger("rclcpp"), "Response received: %d", response[3]);

    readResponse(serial, buf, ec)[3];  // Read the response from motor

	if (ec) {
    RCLCPP_ERROR(rclcpp::get_logger("rclcpp"), "Serial read error: %s", ec.message().c_str());
	} else if (buf.empty()) {
    RCLCPP_ERROR(rclcpp::get_logger("rclcpp"), "Empty response from motor.");
	}


    RCLCPP_INFO(rclcpp::get_logger("rclcpp"), "Motor ID 1 controlled for test.");
	}
