...
//[HexCMD]-This is the actual command identifier for the motors
...
void universialControl(int commandID, int8_t motorID, int degreePosition, int dps, int torque, uint8_t dir, boost::asio::serial_port& serial) {
    ...
    degreePosition = degreePosition * 1000;
    dps = dps * 1000;
    vector<int8_t> command = {/*Header Data ->*/0x3E,hexCMD,motorID,dataSize/*Command Data ->*/ };
    ...
    if (useAngle == 1) {
        ...
        for (size_t i = 0; i < hexString.length(); i += 2) {
            std::string byteString = hexString.substr(i, 2);
            int8_t byte = static_cast<int8_t>(std::stoi(byteString, nullptr, 16));
            bytes.push_back(byte);
        }
        ...
        while (bytes.size() < sizeof64) {
            if(((degreePosition==0)||(degreePosition>0))){
